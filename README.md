...
🚧 Early build. Repo scaffolding ([NOW-4](https://stuartmunn.atlassian.net/browse/NOW-4)), the agent container shell ([NOW-8](https://stuartmunn.atlassian.net/browse/NOW-8)), and both MVP codegen artifact types — Business Rules ([NOW-10](https://stuartmunn.atlassian.net/browse/NOW-10)) and Client Scripts ([NOW-11](https://stuartmunn.atlassian.net/browse/NOW-11)) — are done. Not yet built: now-sdk auth against a real instance (blocked on [NOW-3](https://stuartmunn.atlassian.net/browse/NOW-3) provisioning a dev PDI), the approval flow, deploy-on-approve, and the plugin-facing API — see the [NOW-1 epic](https://stuartmunn.atlassian.net/browse/NOW-1).
...
To run the agent's test suite (exercises both codegen pipelines against a stubbed Claude client, with real `now-sdk build` validation):
